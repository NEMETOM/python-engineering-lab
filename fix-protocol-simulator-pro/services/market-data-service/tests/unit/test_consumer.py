...
class TestRun:
    def test_publishes_when_book_update_received(self):
        with patch("market_data_service.consumer.create_consumer") as mock_create, \
            ...
            book_consumer.poll.side_effect = [_poll_result({"best_bid": 99.0, "best_ask": 101.0}), StopIteration]
    ...
    def test_publishes_when_trade_received(self):
        with patch("market_data_service.consumer.create_consumer") as mock_create, \
            ...
            trade_consumer.poll.side_effect = [_poll_result({"price": 100.0}), StopIteration]
            mock_publisher = mock_publisher_cls.return_value
            with pytest.raises(StopIteration):
                run()
            mock_publisher.publish.assert_called_once()
