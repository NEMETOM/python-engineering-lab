...
class TestSendExecReports:
    def test_emits_two_kafka_messages_per_trade(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        assert mock_exec_producer.send.call_count == 2

    def test_both_messages_go_to_execution_reports_topic(
        self, producer, mock_exec_producer
    ):
        producer.send_exec_reports(_make_trade())
        for c in mock_exec_producer.send.call_args_list:
            assert c[0][0] == "execution_reports"

    def test_buyer_report_has_buy_side(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        buy_reports = [p for p in payloads if p["side"] == "BUY"]
        assert len(buy_reports) == 1

    def test_seller_report_has_sell_side(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        sell_reports = [p for p in payloads if p["side"] == "SELL"]
        assert len(sell_reports) == 1

    def test_buyer_report_has_correct_order_id(self, producer, mock_exec_producer):
    ...
    def test_seller_report_has_correct_order_id(self, producer, mock_exec_producer):
    ...
    def test_buyer_report_has_correct_client_id(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(buy_client_id="FIRM-B"))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        buy_report = next(p for p in payloads if p["side"] == "BUY")
        assert buy_report["client_id"] == "FIRM-B"

    def test_seller_report_has_correct_client_id(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(sell_client_id="FIRM-S"))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        sell_report = next(p for p in payloads if p["side"] == "SELL")
        assert sell_report["client_id"] == "FIRM-S"

    def test_exec_type_is_fill(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["exec_type"] == "F"

    def test_ord_status_is_filled(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["ord_status"] == "2"

    def test_last_px_equals_trade_price(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(price=123.45))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["last_px"] == 123.45

    def test_last_qty_equals_trade_quantity(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(quantity=7))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["last_qty"] == 7

    def test_leaves_qty_is_zero(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade())
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["leaves_qty"] == 0

    def test_symbol_propagated(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(symbol="TSLA"))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        for p in payloads:
            assert p["symbol"] == "TSLA"

    def test_buy_order_qty_propagated(self, producer, mock_exec_producer):
        producer.send_exec_reports(_make_trade(buy_order_qty=50))
        payloads = [c[0][1] for c in mock_exec_producer.send.call_args_list]
        buy_report = next(p for p in payloads if p["side"] == "BUY")
        assert buy_report["order_qty"] == 50

    def test_sell_order_qty_propagated(self, producer, mock_exec_producer):
...
class TestExecReportsBestEffort:
    def test_kafka_error_does_not_raise(self, producer, mock_exec_producer):
