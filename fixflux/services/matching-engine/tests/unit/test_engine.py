...
class TestBuyOrderMatching:
    ...
    def test_buy_full_fill_clears_both_sides(self, engine):
        ...
        assert engine.books[""].buys == []
        assert engine.books[""].sells == []

    def test_buy_partial_fill_remaining_added_to_book(self, engine):
        engine.books[""].add_order(Order("S1", "SELL", 100.0, 5))
        trades = engine.process(Order("B1", "BUY", 100.0, 10))
        assert len(trades) == 1
        assert trades[0].quantity == 5
        assert len(engine.books[""].buys) == 1
        assert engine.books[""].buys[0].quantity == 5
    ...
    def test_buy_matches_multiple_sells(self, engine):
        engine.books[""].add_order(Order("S1", "SELL", 99.0, 5))
        engine.books[""].add_order(Order("S2", "SELL", 100.0, 5))
        trades = engine.process(Order("B1", "BUY", 100.0, 10))
        assert len(trades) == 2

    def test_buy_matches_multiple_sells_total_quantity(self, engine):
        engine.books[""].add_order(Order("S1", "SELL", 99.0, 3))
        engine.books[""].add_order(Order("S2", "SELL", 100.0, 3))
        trades = engine.process(Order("B1", "BUY", 100.0, 10))
    ...
    def test_buy_matches_multiple_sells_remainder_in_book(self, engine):
        ...
        engine.books[""].add_order(Order("S2", "SELL", 100.0, 3))
        engine.process(Order("B1", "BUY", 100.0, 10))
        assert engine.books[""].buys[0].quantity == 4

    def test_buy_matches_cheapest_sell_first(self, engine):
        engine.books[""].add_order(Order("S1", "SELL", 100.0, 5))
        engine.books[""].add_order(Order("S2", "SELL", 99.0, 5))
    ...
    def test_buy_does_not_match_sell_at_higher_price(self, engine):
        engine.books[""].add_order(Order("S1", "SELL", 101.0, 10))
        engine.books[""].add_order(Order("S2", "SELL", 105.0, 10))
        trades = engine.process(Order("B1", "BUY", 100.0, 10))
        assert trades == []
...
class TestSellOrderMatching:
    def test_sell_exact_price_match_produces_one_trade(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 100.0, 10))
    ...
    def test_sell_partial_fill_remaining_added_to_book(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 100.0, 5))
        trades = engine.process(Order("S1", "SELL", 100.0, 10))
        assert len(trades) == 1
        assert trades[0].quantity == 5
        assert len(engine.books[""].sells) == 1
        assert engine.books[""].sells[0].quantity == 5
    ...
    def test_sell_matches_multiple_buys(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 100.0, 5))
        engine.books[""].add_order(Order("B2", "BUY", 99.0, 5))
        trades = engine.process(Order("S1", "SELL", 99.0, 10))
        assert len(trades) == 2

    def test_sell_matches_multiple_buys_total_quantity(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 100.0, 3))
        engine.books[""].add_order(Order("B2", "BUY", 99.0, 3))
        trades = engine.process(Order("S1", "SELL", 99.0, 10))
        assert sum(t.quantity for t in trades) == 6
    ...
    def test_sell_matches_multiple_buys_remainder_in_book(self, engine):
        ...
        assert engine.books[""].sells[0].quantity == 4

    def test_sell_matches_highest_bid_first(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 99.0, 5))
        engine.books[""].add_order(Order("B2", "BUY", 100.0, 5))
    ...
    def test_sell_does_not_match_buy_at_lower_price(self, engine):
        engine.books[""].add_order(Order("B1", "BUY", 95.0, 10))
        engine.books[""].add_order(Order("B2", "BUY", 90.0, 10))
        trades = engine.process(Order("S1", "SELL", 100.0, 10))
        assert trades == []
